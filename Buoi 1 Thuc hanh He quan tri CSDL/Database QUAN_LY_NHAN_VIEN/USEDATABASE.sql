...
/*****************************CAC CAU TRUY VAN**************************/
--1. Hãy cho biết mã nhân viên, tên nhân viên của những nhân viên đã tham gia đề án với thời gian được phân công từ 20 trở lên.
select MANV,TENNV
FROM NHANVIEN N INNER JOIN PHANCONG P
ON N.MANV=P.MA_NVIEN
WHERE THOIGIAN>=20
--2. Hãy cho biết mã nhân viên, tên nhân viên, tên thân nhân và quan hệ thân nhân của tất cả nhân viên, nếu nhân viên không có thân nhân thì tên thân nhân và quan hệ thân nhân là Null (LEFT OUTER)
SELECT N.MANV, T.TENTN, T.QUANHE
FROM NHANVIEN N LEFT JOIN THANNHAN T
ON T.MA_NVIEN=N.MANV
--3. Hãy cho biết mã nhân viên, tên nhân viên, số đề án, thời gian những nhân viên đã tham gia đề án ‘Tin hoc hoa’, những nhân viên không có tham gia đề án ‘Tin hoc hoa’ thì số đề án và thời gian là Null. (LEFT OUTER, nhiều bảng, có điều kiện) 
SELECT MANV,TENNV,SODA,THOIGIAN
FROM (NHANVIEN N 
LEFT OUTER JOIN PHANCONG P ON N.MANV = P.MA_NVIEN)
LEFT OUTER JOIN DEAN D ON P.SODA = D.MADA
WHERE D.TENDA='Tin hoc hoa' OR D.TENDA IS NULL
--4. Tạo View tên V_PB, hãy cho biết mã phòng, tên phòng ban của những phòng ban có địa điểm phòng ban ở ‘VUNG TAU’ hoặc ‘TP HCM’. 
CREATE VIEW V_PB 
AS
SELECT P.MAPHG, P.TENPHG
FROM PHONGBAN P
INNER JOIN DIADIEM_PHG D ON P.MAPHG = D.MAPHG
WHERE D.DIADIEM IN ('VUNG TAU', 'TP HCM');
--5. Tạo View tên V_HCM, hãy cho biết mã nhân viên, tên nhân viên, ngày sinh của những nhân viên có địa chỉ ở thành phố Hồ Chí Minh (TP HCM) hoặc có lương lớn hơn 25000
CREATE VIEW V_HCM 
AS
SELECT MANV,TENNV,NGSINH
FROM NHANVIEN N 
WHERE N.DCHI='TP HCM' OR N.LUONG>25000
--6. Tạo View tên V_SLNV, hãy cho biết mã đề án, tên đề án, số lượng nhân viên tham gia thực hiện từng đề án. 
CREATE VIEW V_SLNV 
AS
SELECT D.MADA, D.TENDA, COUNT(P.MA_NVIEN) AS SOLUONG_NHANVIEN
FROM DEAN D LEFT JOIN PHANCONG P 
ON D.MADA = P.SODA
