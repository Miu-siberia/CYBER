USE QUANLYDONHANG_B1_2374802010353
...
-------------------------------------------------------------------------------------------
--Ví dụ 1:Hãy cho biết mã khách hàng, tên khách hàng, số hóa đơn, ngày đăt hàng của những khách hàng đã từng và chưa từng mua hàng của Công ty. Nếu khách hàng chưa mua thì số hóa đơn, ngày đặt hàng là Null.
select *
FROM MATHANG M left JOIN CHITIETDATHANG C
ON M.mahang=C.mahang

---Ví dụ 2:		Hãy cho biết mã nhân viên, tên nhân viên, số hóa đơn, ngày đăt hàng của những nhân viên đã từng và chưa từng bán hàng cho Công ty. Nếu nhân viên chưa bán thì số hóa đơn, ngày đặt hàng là Null.
SELECT N.MANHANVIEN,TEN,sohoadon,NGAYDATHANG
FROM dondathang D LEFT JOIN nhanvien N
ON N.manhanvien=D.manhanvien
---Ví dụ 3:		Hãy cho biết số hóa đơn, ngày đặt hàng, mã hàng, giá bán của những đơn đặt hàng đã có và chưa có mặt hàng trong chi tiết đặt hàng, hoặc những mặt hàng trong chi tiết đặt hàng nhưng chưa được lập đơn đặt hàng. 
SELECT D.sohoadon,ngaydathang,mahang,giaban
FROM dondathang D FULL OUTER JOIN chitietdathang C
ON D.sohoadon=C.sohoadon
--Ví dụ 4:		Hãy cho biết mã hàng, tên hàng, số hóa đơn, ngày đăt hàng của những mặt hàng đã từng được đặt hàng. 	
SELECT M.MAHANG, TENHANG, D.SOHOADON, NGAYDATHANG 
FROM (MATHANG M  
INNER JOIN CHITIETDATHANG C ON M.MAHANG = C.MAHANG)  
INNER JOIN DONDATHANG D ON D.SOHOADON = C.SOHOADON 
--Ví dụ 5:		Hãy cho biết mã nhà cung cấp, tên nhà cung cấp của những nhà cung cấp có mặt hàng đã từng được đặt hàng.
SELECT N.MANHACUNGCAP,TENNHACUNGCAP
FROM nhacungcap N RIGHT JOIN mathang M
ON N.manhacungcap=M.manhacungcap
--Ví dụ 6:		Hãy cho biết mã hàng, tên hàng của những mặt hàng đã từng được đặt hàng trong tháng 11. 
SELECT M.mahang,tenhang
FROM (MATHANG M  
INNER JOIN CHITIETDATHANG C ON M.MAHANG = C.MAHANG)  
INNER JOIN DONDATHANG D ON D.SOHOADON = C.SOHOADON  
AND MONTH(NGAYDATHANG) = 11 
--Ví dụ 7:		Hãy cho biết mã nhân viên, tên nhân viên của những nhân viên đã từng lập những đơn hàng có bán mặt hàng “Bàn máy tính”
SELECT N.MANHANVIEN,ten
FROM (nhanvien N INNER JOIN DONDATHANG D ON N.manhanvien=D.manhanvien) INNER JOIN chitietdathang C ON D.sohoadon = C.sohoadon INNER JOIN MATHANG M ON C.mahang = M.mahang
WHERE M.TENHANG='Bàn máy tính'
