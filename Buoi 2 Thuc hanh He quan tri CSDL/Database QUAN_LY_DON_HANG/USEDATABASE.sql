USE QUANLYDONHANG_B2_2374802010353
...
--§ Tên:   sp_ThemLoaiHang 
--§ Nhập: @maloaihang, @tenloaihang 
--§ Nội dung: Các bước thực hiện 
--i. Kiểm tra tên loại hàng không trùng trong cơ sở dữ liệu, nếu trùng thì thông báo 
--lỗi và chấm dứt thủ tục. 
--ii. Nếu điều kiện trên thỏa thì thêm mới loại hàng và thông báo thêm thành công
CREATE PROCEDURE sp_ThemLoaiHang
    @maloaihang INT,
...
BEGIN
    IF EXISTS (SELECT * 
	FROM LoaiHang 
	WHERE tenloaihang = @tenloaihang)
    BEGIN
        PRINT 'Lỗi: Tên loại hàng đã tồn tại trong cơ sở dữ liệu.'
        RETURN
    END
    INSERT INTO LoaiHang (maloaihang, tenloaihang)
    VALUES (@maloaihang, @tenloaihang)
    PRINT 'Thêm loại hàng thành công.'
END
--2. Viết một Stored Procedure:  
--§ Tên:   sp_MatHangBanChay 
--§ Nhập: @mahang 
--§ Nội dung: Nhập vào mã hàng, liệt kê thông tin của những đơn hàng (SOHOADON, 
--TENKHACHHANG, MANHANVIEN, NGAYDATHANG) đã từng bán mặt hàng 
--nhập vào với giá bán từ 3000000 trở lên (3 triệu). 
create proc sp_MatHangBanChay
	@mahang varchar(6)
...
begin 
	...
	begin 
		...
		values (@mahang, 'Unknown')
		print 'Nhập vào mã hàng thành công'
	end
	else
	begin
		print'Mã hàng đã tồn tại không cần thêm'
	end
select d.sohoadon,k.tenkhachhang,manhanvien,ngaydathang
from dondathang d inner join chitietdathang c
on d.sohoadon= c.sohoadon inner join khachhang k 
on d.makhachhang=k.makhachhang
where  c.mahang=@mahang and c.giaban>3000000
end

--3. Viết một Stored Procedure:  
--§ Tên:   sp_MatHangGiamGia 
--§ Nhập: @mucgiamgia (mặc định = 5)  
--§ Nội dung: Nhập vào mức giảm giá, liệt kê thông tin của những mặt hàng 
--(MAHANG, TENHANG, MANHACUNGCAP, TENLOAIHANG, SOLUONG) 
--đã từng được bán với mức giảm giá bằng mức giảm giá nhập vào.
create proc sp_MatHangGiamGia
@mucgiamgia int = 5
as
begin
    SELECT 
		...
		C.SOLUONG
	FROM 
		mathang M INNER JOIN loaihang L
		ON M.maloaihang=L.maloaihang INNER JOIN chitietdathang C
		ON C.mahang=M.mahang
    where c.mucgiamgia = @mucgiamgia;
end
...
--§ Nhập: @nam 
--§ Nội dung: Nhập vào năm, liệt kê thông tin những nhân viên (MANHANVIEN, 
--HO, TEN, NGAYSINH, DIACHI, LUONGCOBAN) có ngày sinh lớn hơn ngày 
--sinh nhập vào. 
create procedure sp_NhanVienLauNam
    @nam date
as
begin 
	select manhanvien,ho,ten,ngaysinh,diachi,luongcoban
	from nhanvien
	where ngaysinh > @nam
end
